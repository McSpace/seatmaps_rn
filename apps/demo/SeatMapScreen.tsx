/**
 * SeatMap screen — full-screen seatmap for a single flight.
 *
 * Receives a FlightOption via route.params.flight and fetches/renders the
 * seatmap using the sandbox API. Demonstrates:
...
export default function SeatMapScreen({ route }: Props) {
  ...
  useEffect(() => {
    ...
    apiPost<any[]>('flight/features/plane/seatmap', flight.request)
      .then((response: any[]) => {
        const raw = response[0];
        // ── DEBUG: plane / nose info from API ──────────────────────────────
        console.log(`[API] top-level keys: ${Object.keys(raw).join(', ')}`);
        console.log(`[API] plane:`, JSON.stringify(raw.plane ?? null));
        console.log(`[API] seatDetails.plane:`, JSON.stringify(raw.seatDetails?.plane ?? null));
        console.log(`[API] noseType (top):`, raw.noseType ?? 'undefined');
        console.log(`[API] noseType (seatDetails):`, raw.seatDetails?.noseType ?? 'undefined');
        // ───────────────────────────────────────────────────────────────────
        ...
        // ── DEBUG: params after prepareData ────────────────────────────────
        console.log(`[Prepared] params.noseType="${prepared.params.noseType}" decks=${prepared.content.length}`);
        // ───────────────────────────────────────────────────────────────────
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>{flight.route}</Text>
