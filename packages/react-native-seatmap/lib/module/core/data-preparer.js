...
import { CLASS_CODE_MAP, LOCALES_MAP, ENTITY_SCHEME_MAP, ENTITY_STATUS_MAP, ENTITY_TYPE_MAP, DEFAULT_DECK_TITLE_HEIGHT, DEFAULT_INDEX_ROW_HEIGHT, SEAT_SIZE_BY_TYPE } from './constants';
...
export class JetsContentPreparer {
  ...
  _prepareSeat = (seat, row, cabinFeatures, config) => {
    ...
    const classType = (config.customCabinTitles?.[classCode] ?? config.customCabinTitles?.[classCode?.toUpperCase()]) || CLASS_CODE_MAP[classCode.toLowerCase()] || '';
    ...
    let status = seat.status || ENTITY_STATUS_MAP.available;
    if (config.availabilityMap) {
      if (seatNumber in config.availabilityMap) {
        status = config.availabilityMap[seatNumber] ? ENTITY_STATUS_MAP.available : ENTITY_STATUS_MAP.unavailable;
      }
    }
    ...
    const seatColor = JetsDataHelper.calculateSeatColorByScore(seat?.score, config.colorTheme?.customSeatColorRanges ?? []) || seat?.color;
    return {
      uniqId: Utils.generateId(),
      ...seat,
