...
exports.JetsDataHelper = void 0;
var _constants = require("./constants");
class JetsDataHelper {
  getSeatMapParams = (decks, config, plane) => {
    const decksWidths = decks.map(deck => deck.width);
    const maxDeckWidth = Math.max(...decksWidths) + (config?.colorTheme?.fuselageStrokeWidth ?? 0) * 2 || 0;
    ...
    const rotationCoefs = this._calculateSeatMapRotation(config.horizontal, config.rightToLeft, scaleCoefs.scale, config.scaleType);
    const hullSize = config.visibleFuselage ? maxDeckWidth * _constants.FUSELAGE_HEIGHT_TO_WIDTH_RATIO : 0;
    ...
    const totalDecksHeight = decks.map(deck => deck.height).reduce((a, b) => a + b, 0) + hullSize + separatorSize + deckSpacings;
  ...
  getDeckInnerWidth(biggestRowWidth, config) {
    return biggestRowWidth + _constants.DEFAULT_DECK_PADDING_SIZE * 2 + _constants.THEME_FUSELAGE_OUTLINE_WIDTH * 2 || config.width;
  }
  ...
  findWidestDeckRow = rows => {
    const sorted = [...rows].filter(r => !!r.number).sort((a, b) => b.width - a.width);
    return sorted[0];
  };
  ...
  assignAllLettersForBiggestRow = (biggestRow, rows) => {
    const biggestRowCopy = {
      ...biggestRow,
      seats: biggestRow.seats.map(seat => ({
        ...seat
      }))
    };
    try {
  ...
  rowLetters = row => {
    const knownElementTypes = {
      [_constants.ENTITY_TYPE_MAP.aisle]: '-',
      [_constants.ENTITY_TYPE_MAP.empty]: ' '
    };
  ...
  _calculateSeatMapRotation = (isHorizontal, isRtl, scale, scaleType) => {
    ...
    if (isHorizontal) {
      ...
      offset = scaleType === _constants.SCALE_TYPES.ZOOM ? `translateY(${-100 / scale}%)` : 'translateY(-100%)';
  ...
  calculateDeckHeight = (rows, deckBulks, deckExits) => {
    ...
    const {
      topOffset: lastRowTopOffset,
      seats: lastRowSeats
    } = lastRow;
    const lowestSeat = this._findLowestSeat(lastRowSeats);
    const {
      height: lastBulkHeight,
      topOffset: lastBulkTopOffset
    } = this._calculateLastElementHeight(deckBulks);
    const {
      height: lastExitHeight,
      topOffset: lastExitTopOffset
  ...
  static validateLanguage = lang => {
    if (!lang) return _constants.DEFAULT_LANG;
    const _lang = lang.toUpperCase();
    return _constants.LOCALES_MAP[_lang] ? _lang : _constants.DEFAULT_LANG;
  };
  static _isColor(strColor) {
    ...
    const namedColors = new Set(['red', 'blue', 'green', 'black', 'white', 'transparent', 'dimgrey', 'dimgray', 'lightgray', 'lightgrey', 'darkgrey', 'darkgray', 'grey', 'gray', 'orange', 'yellow', 'purple', 'pink', 'brown', 'cyan', 'magenta', 'lime', 'navy', 'teal', 'silver', 'gold', 'indigo', 'violet', 'coral', 'salmon', 'khaki', 'ivory', 'beige', 'tan']);
    return namedColors.has(strColor.toLowerCase());
  }
  static mergeColorThemeWithConstraints = (defaultTheme, theme) => {
    const merged = {
      ...defaultTheme,
      ...this._filterInvalidColors(theme)
    };
  ...
  static _applyColorRangesConstraints(colorRanges) {
    ...
    return colorRanges.filter(rangeItem => {
      if (!rangeItem || !Array.isArray(rangeItem.range) || rangeItem.range.length !== 2) return false;
...
exports.JetsDataHelper = JetsDataHelper;
